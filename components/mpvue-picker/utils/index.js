...
const getDatePickerIndex = pickerIndex => {
  ...
  if (
    Object.prototype.toString.call(pickerIndex) === "[object Date]" &&
    !isNaN(pickerIndex.getTime())
  ) {
    ...
    if (
      isNaN(
        new Date(
          `${pickerIndex[0]}/${pickerIndex[1]}/${pickerIndex[2]}`
        ).getTime()
      )
    ) {
      tempPickerIndex = transformDateToIndex(); // 兼容错误日期
    } else {
      tempPickerIndex = transformDateToIndex(
        new Date(`${pickerIndex[0]}/${pickerIndex[1]}/${pickerIndex[2]}`)
      );
    }
  }
...
const fixPickerValueDefault = (pickerValue, mode, pickerValueArray) => {
  ...
  if (mode === "selector") {
    // 单列
    fixPickerValue[0] = Math.min(
      fixPickerValue[0],
      pickerValueArray.length - 1
    );
    ...
    fixPickerValue = fixPickerValue.map((item, index) => {
      if (index === 0) {
        ...
        tempIndexSecond = Math.min(
          item,
          pickerValueArray[tempIndexFirst].children.length - 1
        );
        return tempIndexSecond;
      } else if (index === 2) {
        return Math.min(
          item,
          pickerValueArray[tempIndexFirst].children[tempIndexSecond].children
            .length - 1
        );
      }
    });
